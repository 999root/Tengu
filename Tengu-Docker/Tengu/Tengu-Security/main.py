...
BASE_DIR = os.path.join(os.getcwd(), '', 'app', 'lists')
...
def get_all_files_and_directories(path):
    ...
    files = [item for item in items if os.path.isfile(os.path.join(path, item)) and item.endswith('.txt')]
...
def new_file(subpath=''):
    ...
    return render_template('new_file.html', subpath=subpath)
